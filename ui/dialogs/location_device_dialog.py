...
class LocationDeviceDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # --- DEĞİŞİKLİK 1: Sütun sayısı 5'e düşürüldü ---
        self.device_table = QTableWidget(0, 5)
        
        # --- DEĞİŞİKLİK 2: "Tür" başlığı kaldırıldı ---
        self.device_table.setHorizontalHeaderLabels(["ID", "Model", "Seri Numarası", "Renk Tipi", "CPC"])
        ...
        self.device_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)  # Model (Otomatik genişler)
        self.device_table.setColumnWidth(2, 150)  # Seri No
        
        # --- DEĞİŞİKLİK 3: Tür sütunu genişlik ayarı silindi, indeksler kaydırıldı ---
        self.device_table.setColumnWidth(3, 120)  # Renk Tipi (Eskiden 4'tü)
        self.device_table.setColumnWidth(4, 80)   # CPC (Eskiden 5'ti)
    ...
    def _create_edit_panel(self):
        ...
        self.color_type_combo.currentIndexChanged.connect(self.toggle_color_price_field)
    ...
    def load_devices(self):
        ...
        try:
            ...
            for device in devices:
                ...
                # --- DEĞİŞİKLİK 4: "device_type" listeden çıkarıldı ---
                row_data = [
                    device['id'],
                    device['device_model'] or '',
                    device['serial_number'] or '',
                    # device['device_type'] or '', # SİLİNDİ
                    device['color_type'] or '',
                    device['is_cpc'] if device['is_cpc'] is not None else False
                ]

                for col, value in enumerate(row_data):
                    # --- DEĞİŞİKLİK 5: CPC artık 4. indeks (0,1,2,3,4) ---
                    if col == 4:  # CPC kolonu
    ...
    def toggle_color_price_field(self):
        ...
        is_cpc = self.is_cpc_combo.currentText() == "Evet"
        is_color_device = self.color_type_combo.currentText() == "Renkli"
    ...
    def save_device(self):
        ...
        selected_color_type = self.color_type_combo.currentText()

        try:
            device_data = {
                "device_model": self.model_input.text(),
                "serial_number": self.serial_input.text(),
                "device_type": selected_color_type, 
